...
import com.gtemate.petiteannoncekmer.repository.AuthorityRepository;
import com.gtemate.petiteannoncekmer.repository.PersistentTokenRepository;
...
import com.gtemate.petiteannoncekmer.security.SecurityUtils;
import com.gtemate.petiteannoncekmer.service.util.RandomUtil;
...
public class UserService extends BaseEntityService<User>{
    ...
    public boolean isCurrentUserAdmin() {
        Optional<User> optionalUser = getUserWithAuthoritiesByLogin(SecurityUtils.getCurrentUserLogin());
        User user = null;
        boolean isAdmin = false;
        if(optionalUser.isPresent()){
            user = optionalUser.get();
            Authority authority = authorityRepository.findOne(AuthoritiesConstants.ADMIN);
            isAdmin =  user.getAuthorities().contains(authority);
            log.debug("Changed password for User: {}", user);
        }
        return isAdmin;
    }
